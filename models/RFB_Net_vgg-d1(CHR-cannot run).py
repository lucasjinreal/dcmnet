    ...
    def forward(self, x):
        ...
        if self.up_size > 0:  # 2 forward up_size
            x = F.upsample(input=x, size=(
                self.up_size, self.up_size), mode='bilinear')
        return x
...
class RFBNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for k, v in enumerate(self.sources):  # 用enumerate函数k,v键值对将sources列表里存的值取出调用
            # 以v(sources[k]的方式传入中间列表transformed_features
        ...
        # 将新定义的特征层pyramid_ext按键-值对传给pyramid_fea
        for k, v in enumerate(self.pyramid_ext):
            x = v(x)
            pyramid_fea.append(x)
        # print(pyramid_fea[2])
        # pyramid_fea[0]（256*3,512），pyramid_fea[1]（512,512）
        l4_3 = self._upsample_add(pyramid_fea[1], pyramid_fea[0])
        ...
        if self.phase == "test":
            output = (
                # [batch, num_boxes, 4], [1, 8732, 4]                  # loc preds
    ...
    # 区分rfbnet里面的base是vgg16，而ssd_resnet101里面的base是resnet101
...
def add_extras():  # 纯手工定义各层extras
    ...
    layers += [BasicRFB(1024, 512, stride=2, scale=1.0,
                        visual=2)]  # 第三层10*10*512
...
def feature_transform_module(add_extras, size):  # +
    ...
    layers += [BasicConv(add_extras[1].out_channels, 256,
                         kernel_size=1, padding=0, up_size=up_size)]
    return add_extras, layers


# 合并后生成新特征图，经过layers中的卷积后，特征图的尺寸依次为：38*38，19*19，10*10，5*5，3*3，1*1
# + 类似FPN里面的latlayers侧面层，在上采样操作前对自下向上提取的原始特征进行一个初步操作
