    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch0 = nn.Sequential(
            ...
            BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3,
                      stride=1, padding=visual, dilation=visual, relu=False)
        ...
        self.branch3 = nn.Sequential(  # 新加的一个5*5分支，视野也是5
            BasicConv(in_planes, inter_planes,
                      kernel_size=1, stride=1),  # 1024,128
            BasicConv(inter_planes, (inter_planes // 2) * 3,
                      kernel_size=3, stride=1, padding=1),  # 128,192
            BasicConv((inter_planes // 2) * 3, 2 * inter_planes,
                      kernel_size=3, stride=stride, padding=1),  # 192,256
            BasicConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1,
                      padding=2 * visual + 1, dilation=2*visual+1, relu=False)  # 256,256
        )  # visual = 2

        # 4个branch加起来输出,ConvLinear回到了输入和输出通道相同
        self.ConvLinear = BasicConv(
            8*inter_planes, out_planes, kernel_size=1, stride=1, relu=False)
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch2 = nn.Sequential(
            BasicConv(in_planes, inter_planes,
                      kernel_size=1, stride=1),  # 1024,128
            #BasicConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),
            BasicConv(inter_planes, inter_planes+32, kernel_size=(1,
                                                                  3), stride=1, padding=(0, 1)),  # 128,160
            BasicConv(inter_planes+32, inter_planes+64, kernel_size=(3,
                                                                     1), stride=stride, padding=(1, 0)),  # 160,192
            # BasicConv(inter_planes+64, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),#192,256
            BasicConv(inter_planes+64, inter_planes + 96,
                      kernel_size=(1, 3), stride=1, padding=(0, 1)),  # 192,224
            BasicConv(inter_planes + 96, 2 * inter_planes,
                      kernel_size=(3, 1), stride=stride, padding=(1, 0)),  # 224,256
            BasicConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 1,
...
class BasicRFB_a(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1):
        ...
        self.branch3 = nn.Sequential(
            BasicConv(in_planes, inter_planes//2,
                      kernel_size=1, stride=1),  # 512,64
            BasicConv(inter_planes//2, (inter_planes//4)*3,
                      kernel_size=(1, 3), stride=1, padding=(0, 1)),  # 64,96
            BasicConv((inter_planes//4)*3, inter_planes, kernel_size=(3,
                                                                      1), stride=stride, padding=(1, 0)),  # 96,128
            BasicConv(inter_planes, inter_planes, kernel_size=3,
                      stride=1, padding=5, dilation=5, relu=False)  # 128,128
        )
